  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await makeAuthenticatedGETRequest(
        "/appointment/get/appointment/660c091d88755fef0e364e2d"
      );
        ...
        {acceptAppointment && (
          <div>
            <AcceptAppointment
              ...
              patientId={patientId}
              closeModel={() => setAcceptAppointment(false)}
            />
          </div>
        )}
        ...
        {rejectAppointment && (
          <div>
            <RejectAppointment
              ...
              appointmentId={appointmentId}
              patientId={patientId}
              closeModel={() => setRejectAppointment(false)}
            />
          </div>
        )}
        ...
        <div className="w-full h-full flex">
          <div className="w-3/12 m-3 border-2 rounded-xl">
            <div className="border-b pb-6">
              {doctorImage ? (
                <div
                  className="w-full h-32 flex items-center justify-center rounded-t-lg pt-28"
            ...
            <div className="flex items-center justify-center">
              <table className="table-auto ml-10 rounded-xl">
                ...
                {data ? (
                    ...
                    {tableDetail.map((item) => {
                      return (
                        <tr className="border-2">
                          ...
                          <td className="pl-3">
                            {item.patient ? item.patient.age : "N/A"}
                          </td>
                          <td className="pl-3">
                            {item.patient ? item.patient.gender : "N/A"}
                          </td>
                          <td className="pl-3">{item.department}</td>
                          <td className="pl-3">
                            {item.patient ? item.patient.birthDate : "N/A"}
                          </td>
                          ...
                          <td className="flex items-center justify-center space-x-6 mx-6 pt-4">
                            ...
                            >
                              <Icon
                                className="text-2xl cursor-pointer shadow-xl hover:bg-blue-600 hover:text-white duration-300 text-blue-600 bg-blue-100 p-2 rounded-full"
                                icon={"ph:eye"}
                              ></Icon>
                            </p>
                            <p
                            ...
                            >
                              <Icon
                                className="text-2xl cursor-pointer shadow-xl hover:bg-green-600 hover:text-white duration-300 text-green-600 bg-green-100 p-2 rounded-full"
                            ...
                            >
                              <Icon
                                className="text-2xl cursor-pointer shadow-xl hover:bg-red-600 hover:text-white duration-300 text-red-600 bg-red-100 p-2 rounded-full"
                  ...
                  <div>No appointment booked yet</div>
                )}
              </table>
            </div>
          </div>
